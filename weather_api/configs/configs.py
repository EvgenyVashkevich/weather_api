"""This module contains settings containers.
Different settings are stored in different containers, main access is
...
__all__ = ["Configs", "PathConfigs", "ServerConfigs", "WeatherConfigs"]
...
class ServerConfigs(BaseModel):
    ...
    def from_context(cls, context: Context) -> "ServerConfigs":
        return cls(
            ...
            enable_cors=context.env.get(
                "ENABLE_CORS", default=context.yml["server"]["enable_cors"], cast=bool
            ),
        )
...
class WeatherConfigs(BaseModel):
    ...
    def from_context(cls, context: Context) -> "WeatherConfigs":
        return cls(api_key=SecretStr(context.env.get("API_KEY")), **context.yml["weather"])
...
class RedisConfigs(BaseModel):
    ...
    def from_context(cls, context: Context):
        return cls(
            host=context.env.get("REDIS_HOST", default=context.yml["redis"]["host"], cast=str),
            port=context.env.get("REDIS_PORT", default=context.yml["redis"]["port"], cast=int),
            db=context.yml["redis"]["db"],
            encoding=context.yml["redis"]["encoding"],
            hijack_root_logger=context.yml["redis"]["hijack_root_logger"],
        )
...
class Configs:
    ...
    def __init__(self, package_dir: str | Path, **kwargs) -> None:
        ...
        self.redis = RedisConfigs.from_context(self._context)
    ...
    def configure_logging(self) -> "Configs":
        ...
        if "handlers" in self._context.yml["logging"]:
            for key in self._context.yml["logging"]["handlers"]:
                ...
                if handler_fname:
                    self._context.yml["logging"]["handlers"][key]["filename"] = (
                        f"{self.path.logs}/{handler_fname}"
                    )
