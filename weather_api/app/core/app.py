...
class App:
    def __init__(self, configs: Configs) -> None:
        ...
        )  # trick to handle documentation hosting
        ...
        if configs.path and configs.path.static and configs.path.static.exists():
            self.server.mount(
                "/static", StaticFiles(directory=configs.path.static.as_posix()), name="static"
            )

    def enable_cors(self) -> "App":
        """Enable CORS for all origins, methods and headers.
        Do not enable CORS on production servers
    ...
    def render_template(
        self, template: str, request: Request, status_code: int = status.HTTP_200_OK, **kwargs
    ):
        """Render specified `template` with data from `kwargs`"""

        kwargs.update(request=request, urlfor=self.url_for)
        return self.templates.TemplateResponse(
            name=template, context=kwargs, status_code=status_code
        )

    def include_routers(self) -> "App":
        """Main application method: connect all exceptions handlers,
        endpoints and blueprints to app"""
